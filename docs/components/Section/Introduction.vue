...
<template>
  <PageSection>
    ...
    values or to transfer values from one array to another. Unlike most
    comparable packages, Drag and Drop does not perform any direct DOM
    manipulations to do so. Instead, it takes a declarative approach by updating
    state and allowing the reactive framework (of your choosing) to handle the
    rest.
    ...
    <h2>Features:</h2>
    <br />
    <h4>Sorting (Ranking something 80s related.)</h4>
    <p>
    ...
    <p>
      To enable transfer functionality between two parents, set the `group` in
      the config object of each to be the same.
    </p>
    <br />
    <h4>Drag handles</h4>
    <br />
    <h4>
      Accepts (Accepts a callback function that returns a boolean to determine
      if transfer operation should be allowed)
    </h4>
    <br />
    <h4>
      Disabled (show it being enabled, and then on button press, you can't drag
      any more of its items.)
    </h4>
    <br />
    <h4>Draggable</h4>
    <p>
      There are many cases where you would not want all the immediate children
      of the parent to be sortable. In these cases, you pass a `draggable`
